from ast_nodes import *

class Parser:
    def __init__(self, tokens):
        self.tokens = tokens
        self.pos = 0

    def current_token(self):
        """Helper to peek at the current token without moving the pointer."""
        if self.pos < len(self.tokens):
            return self.tokens[self.pos]
        return None

    def current(self):
        """Alias for current_token to maintain compatibility with expression logic."""
        return self.current_token()

    def eat(self, type_):
        token = self.current_token()
        if token and token[0] == type_:
            self.pos += 1
            return token
        raise Exception(f"Expected {type_}, got {token}")

    def parse(self):
        stmts = []
        while self.current_token():
            stmts.append(self.statement())
        return stmts

    def statement(self):
        tok = self.current_token()
        if not tok: return None

        if tok[0] == "ID":
            node = self.assignment()
            self.eat("SEMI") # Standard assignments still need semicolons
            return node
        elif tok[0] == "PRINT":
            return self.print_stmt()
        elif tok[0] == "INPUT":          
            return self.input_stmt()
        elif tok[0] == "IF":
            return self.if_stmt()
        elif tok[0] == "WHILE":
            return self.while_stmt()
        elif tok[0] == "FOR":              
            return self.for_stmt()
        else:
            raise Exception(f"Invalid statement: {tok}")

    def assignment(self):
        """Handles both variable (x = 5) and array (x[0] = 5) assignments."""
        name = self.eat("ID")[1]
        
        if self.current_token() and self.current_token()[0] == "LBRACK":
            self.eat("LBRACK")
            index = self.expr()
            self.eat("RBRACK")
            self.eat("EQ")
            val = self.expr()
            return ArrayAssign(name, index, val)
        else:
            self.eat("EQ")
            val = self.expr()
            return Assign(name, val)

    def input_stmt(self):
        self.eat("INPUT")
        name = self.eat("ID")[1]
        
        if self.current_token() and self.current_token()[0] == "LBRACK":
            self.eat("LBRACK")
            index = self.expr()
            self.eat("RBRACK")
            target = ArrayAccess(name, index)
        else:
            target = Var(name)
            
        self.eat("SEMI")
        return Input(target)

    def block(self):
        stmts = []
        self.eat("LBRACE")
        while self.current_token() and self.current_token()[0] != "RBRACE":
            stmts.append(self.statement())
        self.eat("RBRACE")
        return stmts

    def if_stmt(self):
        self.eat("IF")
        condition = self.expr()
        self.eat("THEN")
        body = self.block()

        elifs = []
        while self.current_token() and self.current_token()[0] == "ELSEIF":
            self.eat("ELSEIF")
            elif_cond = self.expr()
            self.eat("THEN")
            elif_body = self.block()
            elifs.append((elif_cond, elif_body))

        else_body = []
        if self.current_token() and self.current_token()[0] == "ELSE":
            self.eat("ELSE")
            else_body = self.block()

        # SEMI is no longer eaten here to allow block-based termination
        return If(condition, body, elifs, else_body)

    def while_stmt(self):
        self.eat("WHILE")
        condition = self.expr()
        self.eat("THEN")
        body = self.block()
        return While(condition, body)
    
    def for_stmt(self):
        self.eat("FOR")
        self.eat("LPAREN")
        
        # 1. Initialization
        init = self.assignment() 
        self.eat("SEMI")
        
        # 2. Condition
        condition = self.expr()
        self.eat("SEMI")
        
        # 3. Update (Note: assignment() is called but its SEMI is eaten in statement())
        # To handle update inside FOR parens, we parse the assignment manually
        update_name = self.eat("ID")[1]
        self.eat("EQ")
        update_val = self.expr()
        update = Assign(update_name, update_val)
        
        self.eat("RPAREN")
        self.eat("THEN")
        body = self.block()
        
        return For(init, condition, update, body)

    def print_stmt(self):
        self.eat("PRINT")
        val = self.expr()
        self.eat("SEMI")
        return Print(val)

    # --- EXPRESSION PARSING ---
    
    def expr(self):
        return self.logical_or()

    def logical_or(self):
        left = self.logical_and()
        while self.current() and self.current()[0] == "OR":
            op = self.eat("OR")[0]
            right = self.logical_and()
            left = BinOp(left, op, right) 
        return left

    def logical_and(self):
        left = self.logical_not()
        while self.current() and self.current()[0] in ("AND", "XOR"):
            op = self.eat(self.current()[0])[0]
            right = self.logical_not()
            left = BinOp(left, op, right)
        return left

    def logical_not(self):
        if self.current() and self.current()[0] == "NOT":
            op = self.eat("NOT")[0]
            right = self.logical_not() 
            return UnaryOp(op, right)
        return self.comparison()

    def comparison(self):
        left = self.arithmetic()
        while self.current() and self.current()[0] in ("GT", "LT"):
            op = self.eat(self.current()[0])[0]
            right = self.arithmetic()
            left = BinOp(left, op, right)
        return left

    def arithmetic(self):
        left = self.term()
        while self.current() and self.current()[0] in ("PLUS", "MINUS"):
            op = self.eat(self.current()[0])[0]
            right = self.term()
            left = BinOp(left, op, right)
        return left

    def term(self):
        left = self.factor()
        while self.current() and self.current()[0] in ("MUL", "DIV"):
            op = self.eat(self.current()[0])[0]
            right = self.factor()
            left = BinOp(left, op, right)
        return left

    def factor(self):
        tok = self.current()
        if not tok: raise Exception("Unexpected end of input")

        if tok[0] == "NUMBER": return Num(self.eat("NUMBER")[1])
        if tok[0] == "STRING": return Str(self.eat("STRING")[1].strip('"'))
        
        if tok[0] == "LBRACK":
            self.eat("LBRACK")
            elements = []
            if self.current() and self.current()[0] != "RBRACK":
                elements.append(self.expr())
                while self.current() and self.current()[0] == "COMMA":
                    self.eat("COMMA")
                    elements.append(self.expr())
            self.eat("RBRACK")
            return ArrayDecl(elements)

        if tok[0] == "ID":
            name = self.eat("ID")[1]
            if self.current() and self.current()[0] == "LBRACK":
                self.eat("LBRACK")
                index = self.expr()
                self.eat("RBRACK")
                return ArrayAccess(name, index)
            return Var(name) 

        if tok[0] == "LPAREN":
            self.eat("LPAREN")
            e = self.expr()
            self.eat("RPAREN")
            return e

        raise Exception(f"Invalid expression: {tok}")